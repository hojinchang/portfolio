...
const ProjectArticle: FC<ProjectArticleProps> = ({ project }) => {
    ...
    return (
        <article 
            ...
            className={`flex bg-neutral-800 mb-10 p-3 rounded-lg md:gap-6 ${isMobile ? "flex-col" : "flex-row"}`}
        >
            ...
            <div className={`flex flex-col gap-8 p-4 pb-2 md:justify-end md:pb-6 ${isMobile ? "" : "w-1/2"}`}>
                ...
                <div className="flex justify-center gap-8 2xs:gap-20 xs:gap-28 sm:gap-36 md:flex-col md:gap-0 md:items-end">
                    <Link to="/" target="_blank" className="underline p-2 lg:text-lg">VIEW MORE</Link>
                    <Link to={project.acf.live_site_link} target="_blank" className="underline p-2 lg:text-lg">LIVE SITE</Link>
                </div>
            </div>
        </article>
    )
